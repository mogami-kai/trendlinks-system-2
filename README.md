...
| 2要素認証 (TOTP) | 認証アプリによる MFA 設定（`/settings/security`） | 全 |
| LINE 連携自動化 | 連携コードを LINE に送信して `line_user_id` を自動紐付け | 全 |
| スタッフ管理 | 招待・ロール変更・有効/無効・自分の作業一覧 | — |
| データ管理 | 配信ログ閲覧・CSV エクスポート（現場/作業指示/報告/取引先） | Admin |
